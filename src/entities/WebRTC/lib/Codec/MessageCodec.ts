...
export class MessageCodec implements Required<MessageCodecConsturcorParams> {
  ...
  constructor(params: MessageCodecConsturcorParams) {
    ...
    this.chunkSize = Math.max(this.headerSize, this.maxChunkSize - this.headerSize)
  ...
  createChunk(data: ArrayBuffer, dataId: Uint8Array, chunkNumber: number, params: object) {
    const header = new Uint8Array(this.headerSize)
    const chunkid = new Uint8Array(this.maxbitdepthForChunkId).map((_, index) => chunkNumber >> (index * 8)).reverse()
    ...
    header.set(headerparams, this.headerNameUTF8.byteLength + this.idLength + chunkid.byteLength)

    const dataChunk = data.slice(chunkNumber * this.chunkSize, (chunkNumber + 1) * this.chunkSize)
  ...
  parseChunk(data: ArrayBuffer) {
    ...
    const dataid = chunkMeta.slice(chunkHeader.byteLength, chunkHeader.byteLength + this.idLength)
    ...
    const chunkparams = chunkMeta
      .slice(chunkHeader.byteLength + dataid.byteLength + this.maxbitdepthForChunkId)
      .filter(Boolean)
